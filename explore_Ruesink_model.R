...
# The idea is to fit a smoothing curve to identify the peaks and then figure out the offset
# You can use degree days (physioloical time) rather than actual time
id = 1
di = d %>% 
  filter(group_id == id) 
dl = di %>%
...
ggplot(data=dl, aes(date0+time, value, color=name)) +
